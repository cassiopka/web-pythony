...
def generate_trunk_config(
    intf_vlan_mapping: Dict[str, List[int]],
    trunk_template: List[str]) -> List[str]:
    result = []
    for (interface, vlans) in intf_vlan_mapping.items():
        result.append(f"interface {interface}")
        for template in trunk_template:
            if "allowed vlan" in template:
                template += ' ' + ','.join([str(vlan) for vlan in vlans])
            result.append(template)
    return result

if __name__ == "__main__":
    print("[", ",\n".join(map(lambda x: '\''+x+'\'', generate_trunk_config(trunk_config, trunk_mode_template))), "]", sep="\n")
