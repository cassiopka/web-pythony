...
class Review(db.Model):
    ...
    created_at = db.Column(
        db.DateTime, nullable=False, server_default=sa.sql.func.now()
    )
    # status_id = db.Column(db.Integer, db.ForeignKey('statuses.id', ondelete='CASCADE'), nullable=False)
    status_id = db.Column(
        db.Integer, db.ForeignKey("statuses.id", ondelete="CASCADE"), nullable=False
    )
    ...
    def get_user(self):
        return (
            db.session.execute(db.select(User).filter_by(id=self.user_id))
            .scalar()
            .login
        )
